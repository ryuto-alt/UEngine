...
void Minimap::UpdateState(const Vector3& playerPos,
    ...
    // Scroll NavMesh map texture to center on player
    if (hasNavMeshMap_ && backgroundSprite_) {
        ...
        if (boundsWidth > 0.0f && boundsHeight > 0.0f) {
            // Player position in texture pixel coordinates
            // Texture has Z flipped (+Z = top of image = row 0)
            float u = (playerPos.x - bounds_.minX) / boundsWidth * mapTexWidth_;
            float v = (bounds_.maxZ - playerPos.z) / boundsHeight * mapTexHeight_;

            // How many texture pixels correspond to the minimap display area
...
void Minimap::Draw() {
    ...
    // Orb counter above minimap using BitmapFont
    int remaining = totalOrbs_ - collectedOrbs_;
    if (bitmapFont_) {
        // Color based on progress
        float pct = totalOrbs_ > 0 ? static_cast<float>(collectedOrbs_) / totalOrbs_ : 0.0f;
        Vector4 color;
        if (pct >= 1.0f)      color = {0.0f, 1.0f, 0.3f, 1.0f};  // Green
        else if (pct >= 0.5f) color = {1.0f, 1.0f, 0.0f, 1.0f};  // Yellow
        else                  color = {1.0f, 0.6f, 0.0f, 1.0f};  // Orange
